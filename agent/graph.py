...
def _strip_thinking(text: str) -> str:
    """Remove Qwen3 <think>...</think> blocks before parsing LLM output."""
...
def _parse_verify_json(content: str) -> tuple[bool, str]:
    """Parse {"ok": bool, "issue": str} from a model response defensively.

    Tries three extraction strategies in order:
    1. Direct json.loads on the whole response
    2. JSON inside a code fence
    3. First bare {...} pattern in the text
    Falls back to ok=False so a failed parse triggers a revise rather than
    silently accepting a bad result.
    """
    content = _strip_thinking(content)

    def _try(s: str) -> tuple[bool, str] | None:
        try:
            data = json.loads(s)
    ...
    if r := _try(content.strip()):
        return r
    m = re.search(r"```(?:json)?\s*(\{.*?\})\s*```", content, re.DOTALL | re.IGNORECASE)
    if m and (r := _try(m.group(1))):
        return r
    m = re.search(r"\{[^{}]*\}", content)
    if m and (r := _try(m.group())):
        return r
    return False, "Could not parse verifier response (see logs)"
